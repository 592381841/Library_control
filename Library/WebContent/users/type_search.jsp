...
//                    $("#blank_div").remove();  
                   ...
                   for(var i=0;i<LAlist.length;i++){
                	   var dto=LAlist[i];
                	   var str1=$("<tr></tr>");
						str1.append("<td><a target='_black' href='<c:url value='/users/jump_book_info.action' />?bookInfoDto.id="+dto.id+"'>"+dto.bookName+" </a></td><td>"+dto.id+" </td><td>"+dto.bookId+" </td><td>"+dto.bookWriter+" </td><td>"+dto.bookPress+" </td>")
...
function pageClick(e){
	...
	if(pageButton=="首页"){
 	 ...
 	 $.ajax({
                cache: false,
                type: "GET",
                url:"<c:url value='/users/json_findBookInfoByBookType.action'/>?pageInfo.page="+page+"&bookTypeDto.botyId="+ $("#typeID").val(),
                async:true,
	  ...
	  function selectBookType(athis){
		  ...
		  $.ajax({
              cache: false,
              type: "GET",
              url:"<c:url value='/users/json_findBookInfoByBookType.action'/>?pageInfo.page=1"+"&bookTypeDto.botyId="+ $("#typeID").val(),
              async:true,
              success:function (data){
              	load_list(data);
              }
