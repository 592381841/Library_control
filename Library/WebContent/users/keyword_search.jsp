                   ...
                   $("#list_tbody").children("tr").remove();
                      ...
                      if(jsonText.bookInfoDtos==null){
                       ...
                       for(var i=0;i<LAlist.length;i++){
                    	   var dto=LAlist[i];
                    	   var str1=$("<tr></tr>");
    						str1.append("<td><a  target='_black' href='<c:url value='/users/jump_book_info.action' />?bookInfoDto.id="+dto.id+"'>"+dto.bookName+" </a></td><td>"+dto.id+" </td><td>"+dto.bookId+" </td><td>"+dto.bookWriter+" </td><td>"+dto.bookTypeDto.bookTypeName+" </td><td>"+dto.bookPress+" </td>")
                    	  	str1.append("<td>"+dto.stackRoomName+"/"+dto.bookRackName+"/"+dto.bookrackNumber+"</td>");
    ...
    source : function (query, process) {
       ...
       if(parameter==null)return;
        $.ajax({
            ...
            type: "POST",
            data:parameter, 
            url:"<c:url value='/users/json_findKeyBookInfo.action'/>",
            async:true,
...
function pageClick(e){
	...
	if(pageButton=="首页"){
 	 ...
 	 $.ajax({
                cache: false,
                type: "GET",
                url:"<c:url value='/users/json_search_page_keyboardBookInfo.action'/>?pageInfo.page="+page,
                async:true,
                success:function (data){
                	  load_list(data);
                }
...
function searchBookInfo(){
 ...
 if(parameters==null)return;
	$.ajax({
        ...
        type: "POST",
        data:parameters, 
        url:"<c:url value='/users/json_search_page_keyboardBookInfo.action'/>",
        async:true,
        success:function (data){
